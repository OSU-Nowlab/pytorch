    ...
    if(OMPI_INFO)
      ...
      if(_output MATCHES "smcuda")
        ...
        if(USE_CUDA_MPI)
          ...
          if(USE_MPIX_STREAM)
            message(WARNING "Forcing MPIX Stream.")
            set(CAFFE2_USE_MPIX_STREAM 1)
          endif()
        else()
      ...
      find_program(MVP_INFO NAMES mpichversion HINTS ${MPI_CXX_LIBRARIES}/../bin)
      if(MVP_INFO)
        execute_process(COMMAND ${MVP_INFO} OUTPUT_VARIABLE _output)
        if(_output MATCHES "with-cuda" OR _output MATCHES "with-rocm")
            message(STATUS "Found MVAPICH-Plus with GPU support built.")
            set(CAFFE2_USE_CUDA_MPI 1)
            if(USE_MPIX_STREAM)
              message(STATUS "Enabling MPIX Stream Extensions")
              set(CAFFE2_USE_MPIX_STREAM 1)
            endif()
        else()
          if(USE_CUDA_MPI)
            message(WARNING "MVAPICH-Plus with GPU support not found, but forcing anyway.")
            set(CAFFE2_USE_CUDA_MPI 1)
            if(USE_MPIX_STREAM)
              message(WARNING "Forcing MPIX Stream anyway.")
              set(CAFFE2_USE_MPIX_STREAM 1)
            endif()
          endif()
        endif()
      else()
        message(WARNING "MVAPICH-Plus found, but it is not built with GPU SUPPORT.")
        set(CAFFE2_FORCE_FALLBACK_CUDA_MPI 1)
      endif()
    endif()
  else()
